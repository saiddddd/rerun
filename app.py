...
def main():
    ...
    aktivitas = ["▪️ About","▪️ EDA", "▪️ EDA Lanjutan","▪️ Plotting", "▪️ Modelling", "▪️ Fraud Detection", "▪️ Clustering","▪️ Stream ML"]
    ...
    if choice == "▪️ About":
        ...
        if data is not None:
            ...
            if st.checkbox("Pilih Variable/Feature tertentu"):
                selected_columns = st.multiselect("Pilih", all_columns)
                new_df = df[selected_columns]
                st.dataframe(new_df)
            if st.checkbox("Basic Description"):
                st.write(df.describe())
            if st.checkbox("Kelas Target"):
                st.write(df.iloc[:,-1].value_counts())    
            if st.checkbox("Korelasi"):
                plt.subplots(figsize=(10,10))
                st.write(sns.heatmap(df.corr(), annot=True))
                st.pyplot()
            if st.checkbox("Pie Chart"):
                all_columns = df.columns.to_list()
                columns_to_plot = st.selectbox("Pilih 1 Variable/Feature", all_columns)
                pie_plot = df[columns_to_plot].value_counts().plot.pie(autopct="%1.1f%%")
                st.write(pie_plot)
                st.pyplot()
    elif choice == "▪️ EDA Lanjutan":
        st.subheader("Melihat Eksplorasi Data Lebih Jauh")
        ...
        if data is not None:
            ...
            if len(all_columns_names) > 0:
                type_of_plot = st.selectbox("Pilih tipe grafik yang akan ditampilkan", ["area","bar","line","hist","box","kde"])
                selected_column_names = st.multiselect("Pilih variabel/Feature yang akan diplot", all_columns_names)
                if st.button("Generate Plot"): #bagian button
                    st.success("Grafik dari pilihan {} untuk data {} akan segera ditampilkan".format(type_of_plot,selected_column_names))
